# Activación de cuentas y piloto curricular v0.11.0
...
## Registro longitudinal en esta versión

Al activar las cuentas, cada residente dispone de un panel **Objective progress**.
El contador muestra desempeños satisfactorios revisados por un docente frente a la
meta del programa. Cada encuentro puede aportar a varios objetivos, evaluados por
separado. Alcanzar la meta detiene nuevas observaciones para ese objetivo; no borra
el encuentro ni impide registrar otros objetivos pendientes.

En **Resident activity and recorded evidence**, el docente abre un encuentro
completado, revisa sus decisiones y registra una valoración por objetivo con
referencias de evidencia, profundidad, autonomía, contexto y comentario. Una orden
reconocida o un buen desenlace no generan crédito automáticamente. Las ocho metas
iniciales corresponden a la tabla aportada por el director del programa, con el
alcance y las restricciones detallados en [Registro por objetivo](OBJECTIVE_TRACKING.md).

La confirmación docente es un paso separado del contador. Las modificaciones de
metas, confirmaciones, reaperturas y anulaciones conservan un motivo y un historial.
La revisión cerrada del encuentro queda congelada también en la base de datos.

Las tablas adicionales se crean automáticamente al abrir el panel de cuentas.
No es necesario borrar ni volver a crear usuarios o encuentros de v0.10.0.
Los encuentros antiguos completados pueden revisarse; no reciben créditos
retroactivos automáticos. La clave común no identifica a los residentes y, por sí
sola, no permite llevar este registro individual.
...
El ZIP completo contiene la carpeta `management_reasoning_simulator_v0.11.0`.

```bash
cd ~/Downloads
unzip management_reasoning_simulator_v0.11.0.zip
...
python -m pytest test_account_store.py test_account_portal.py test_encounter_generator.py test_curriculum_assignment.py test_curriculum_trajectories.py test_curriculum_app.py test_objectives.py test_progress_store.py test_progress_portal.py -q
```

En la entrega v0.11.0 pasaron 99 pruebas, 36 subpruebas y las 56 regresiones activas. Las pruebas cubren aislamiento entre usuarios, roles, invitaciones de un solo uso, expiración y revocación, persistencia local real, conflictos de guardado entre pestañas, recorrido Streamlit, caso congelado, cardioversión, reevaluación y ECG. PostgreSQL está soportado por el código, pero aún debe verificarse contra la base concreta del despliegue. Las pruebas de concurrencia del contador son locales; no equivalen a una prueba de carga del despliegue ni a validación clínica o psicométrica.
